function spl = Focused_Time_depedent_model_12(X,Testdata,K,H_K,burnin,num,space)
...
% Good results for channel_8: hyperparamaters setting:  d0 = 1*10^-6*ones(p,1);h0 = 1*10^-6*ones(p,K);
%    G_lamda{k,Ch}=eye(K); G_mu{k,Ch}=zeros(K,1);alpha0=0.0001;GammaPrior_a0=0.1;
%GammaPrior_a1=0.1;
% Good results for days_8 d0 = 1*10^-4*ones(p,1); h0 = 1*10^-6*ones(p,K);
%G_lamda{k,Ch}=eye(K); G_mu{k,Ch}=zeros(K,1);alpha0=1;alpha1 =300;GammaPrior_a0=0.1;%lambda0
%GammaPrior_a1=0.1;
%-------------------------------------------------
...
for m=1:M 
     n(m)=size(X{m},2);
    for Ch=1:N 
        X{m}(:,:,Ch)=(X{m}(:,:,Ch)-repmat(mean(X{m}(:,:,Ch),2),1,n(m)))*20;
    end
...
p=size(X{1},1);
K=min(K,p);
c0 = 10^-0*ones(p,1); d0 = 1*10^-4*ones(p,1);
...
% F1 = Rmatrix(1,max(n*N));
% logF = log(max(F1,realmin));

z = rand(K,1)<pai;

alpha = gamrnd(g0,1./(h0));
G_v0=K;
G_w0=1*eye(K);%good  result
G_beta0=1;
for Ch=1:N
    for k=1:H_K
    G_lamda{k,Ch}=eye(K);
...
%         spl.TotalLogLik{ndx}=TestdataLogLikelihood_2(Testdata,K,z,w,A,H_v,phi,G_mu,G_lamda);    
        ...
        spl.alpha0=alpha0;
        spl.alpha1=alpha1;
        spl.GammaPrior_a0=GammaPrior_a0;
         spl.GammaPrior_a1=GammaPrior_a1;
    end
        spl.acc=acc;
...
%              end
               figure(100);
               for m=1:M
                   subplot(2,4,m);plot(H_z{m},'r+');
               end
%                figure(888);
%                for m=1:40
%                    subplot(5,8,m);plot(A(:,m),'r+');
%                end
%                figure(999);stem(z);
