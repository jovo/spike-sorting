function [Lambda, rej_ac] = RejSampler_GaNeg_1(Lambda, gamma, B, Stats)

%Lambda is the parameter of interest, rej_ac is the acceptance rate

%gamma is the hyperparameter
...
beta = 1 - sum(bsxfun(@times, B, log(1-p)), 1);
...
%     b(b==Grid) = Grid;
    ...
    LL(:,ml)=bsxfun(@times,gamma-1,log(b(:,ml)))-bsxfun(@times,beta(ml),b(:,ml)) ...
...
while nnz(Rj)>0 && Count_rj < Iter_rj
    ...
    fp = (gamma-1)./lmax(Rj) - beta(Rj) + sum(Comp_1,1);
    Comp_2 = bsxfun(@minus, psi(1,bsxfun(@plus, lmax(Rj),Stats(:,Rj))),psi(1,lmax(Rj)));
    fpp = -(gamma-1)./(lmax(Rj).^2) + sum(Comp_2,1);
...
gamma1 = gamma + sum(Add,1);
gamma2 = beta - Linear + sum(Add,1)./lmax;

Orig_S = sum(bsxfun(@minus,gammaln(bsxfun(@plus, lmax, Stats)),gammaln(lmax)).*B,1)...
    - sum(Add,1).*log(lmax);

if min(gamma2)<0
    disp([num2str(nnz(gamma2<0)),' gammas had scale parameter less than 0']);
end

L_P1 = gamrnd(gamma1,1./gamma2);
