...
Concerning sampling $\phi_m$, since $\phi_m\propto$ $ \prod_{i: b_m^{(i)}=1}$ $\mbox{NegBin}(n^{*}_{im};\phi_m ,p_i)\mbox{Ga}( \phi_m;\gamma_0,1)$ \add{(see Appendix}  \ref{sec:related} \add{ for details)}, using Lemma \ref{lem:NBinference}, we can first sample a latent count variable $\ell_{im}$ for each $n^{*}_{im}$ as
\beq
...
\subfigure[]{
   \includegraphics[width=1.0\linewidth] {figs_new/device.eps}
   \label{fig:device}
...
\add{FMM-DL dominates HDP-DL on these data. }}} \label{fig:long}
\end{figure}
...
\subsection{Connection to Previous Bayesian Non-Parametrics} % (fold)
